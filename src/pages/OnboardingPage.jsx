...
export default function OnboardingPage() {
  ...
  const isManagerRole = ['manager', 'c_suite', 'hr', 'it'].includes(profile?.role)
  ...
  async function handleSubmit(e) {
    ...
    if (!isManagerRole && !selectedManager) { setError('Please select your line manager.'); return }
    setError('')
    setLoading(true)
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {!isManagerRole && (
              <div>
                <label className="label">Line manager <span className="text-red-500">*</span></label>
                ...
                {selectedManager ? (
                  <div className="flex items-center justify-between p-3 rounded-xl border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/30">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-500">({selectedManager.role === 'c_suite' ? 'C-Suite' : 'Manager'})</span>
                    </div>
                      ...
                      {filteredManagers.length === 0 ? (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">{m.email} · {m.role === 'c_suite' ? 'C-Suite' : 'Manager'}</p>
                          </div>
                        </button>
                      ))}
                    </div>
