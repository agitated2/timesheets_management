...
--    Checks the roles array first; falls back to legacy role column for un-migrated rows.
...
--    Manager/C-Suite dropdown (onboarding + settings)
...
CREATE POLICY "profiles_read_for_manager_select" ON public.profiles
  FOR SELECT USING (
    ...
    AND (
      'manager' = ANY(roles) OR 'c_suite' = ANY(roles)
    )
  );

--    Privileged read: HR / C-Suite / IT
